...
``CURLOPT_TLS_TRUST_ANCHORS`` (string)
  ...
  order. Prefix the list with ``fixed:`` to preserve the configured order, or use
  ``none`` to omit the extension. An empty string advertises support without
  requesting a specific trust anchor.
  This option does not change certificate verification or the local CA trust store.
  Command line: ``--tls-trust-anchors <OID list>``.

``CURLOPT_HTTP3_TLS_TRUST_ANCHORS`` (string)
  Overrides ``CURLOPT_TLS_TRUST_ANCHORS`` for QUIC TLS. It accepts the same list,
  ``fixed:`` prefix, empty-string and ``none`` forms. A NULL value inherits the
  TCP setting. Connection matching and TLS session caching include this override.
  Command line: ``--http3-tls-trust-anchors <OID list>``.
...
``CURLOPT_HTTP3_SIG_HASH_ALGS`` (string)
  ...
  With BoringSSL, a leading ``GREASE:`` adds one randomized GREASE signature
  algorithm before the real algorithms. This prefix also works with
  ``CURLOPT_SSL_SIGNATURE_ALGORITHMS`` and ``--sigalgs`` for TCP TLS.
